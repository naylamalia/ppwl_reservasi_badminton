<!-- filepath: c:\laragon\www\uts_ppwl\resources\views\reservations\create.blade.php -->
@extends('layouts.app')
...
<div class="container">
    ...
    <form action="{{ route('reservations.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="courts_id" id="courts_id" class="form-select @error('courts_id') is-invalid @enderror" required>
                ...
                @foreach($courts as $court)
                    <option value="{{ $court->id }}" {{ old('courts_id') == $court->id ? 'selected' : '' }}>{{ $court->id }} - {{ $court->nama }}</option>
                @endforeach
            </select>
            @error('courts_id')
        ...
        <div class="mb-3">
            ...
            <input type="number" name="total_harga" id="total_harga" class="form-control @error('total_harga') is-invalid @enderror" value="{{ old('total_harga') }}" required>
            @error('total_harga')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
